...
// simple per‑project extras (add your content here)
const extras: Record<string, { metrics?: { label: string; value: string }[]; images?: { src: string; alt: string }[]; body?: JSX.Element }> = {
"ai-rota-assistant": {
metrics: [
...
body: (
<>
<p>Early-stage pilot exploring rota automation via WhatsApp + Zapier + OpenAI for SMEs. Focus: reduce back‑and‑forth, increase staffing visibility, and capture simple constraints.</p>
<ul>
...
{proj.stack.map((t) => (
<span key={t} className="rounded-full border border-neutral-200/60 dark:border-neutral-800 px-3 py-1 text-sm opacity-80">{t}</span>
))}
</div>


{/* Metrics */}
{x.metrics && x.metrics.length > 0 && (
...
{/* Case body */}
{x.body && <Section title="Case Study">{x.body}</Section>}
